...
/**
 * 19 — INTERNSHIP
 * Designed for internship & articleship entry-level candidates: Education + skills + achievements + extracurriculars prioritized.
 */
export default function Template19_InternationalIFRS({ cv }) {
  return (
    <div className="w-full bg-white text-zinc-900 font-sans text-[11px] leading-tight select-text min-h-[1050px] p-7 text-left">
      <div className="border-b-2 border-teal-600 pb-4 mb-4 flex justify-between items-center">
        <div className="space-y-1">
          <span className="text-[9.5px] font-bold text-teal-600 uppercase tracking-widest">Articleship & Internship Applicant</span>
          <h1 className="text-2xl font-black text-black">{cv.fullName}</h1>
          <div className="text-xs text-zinc-600 font-semibold">
            {[cv.ftsBatch, cv.crn, cv.targetRole || 'Audit Trainee Applicant'].filter(Boolean).join(' • ')}
          </div>
        </div>
      ...
      <div className="space-y-4">
        {cv.personalStatement && (
          <section>
            <h2 className="text-xs font-black uppercase tracking-wider text-teal-800 border-b border-zinc-200 pb-0.5 mb-1">
        ...
        {cv.academics && cv.academics.length > 0 && (
          <section>
            <h2 className="text-xs font-black uppercase tracking-wider text-teal-800 border-b border-zinc-200 pb-0.5 mb-1.5">
              ...
              {cv.academics.map((acad, idx) => (
                <div key={idx} className="flex justify-between items-center text-[10.5px] border-b border-zinc-100 pb-1">
                  ...
                  <div className="text-right">
                    <span className="font-bold text-teal-800">{acad.year}</span> | <span className="font-semibold text-emerald-700">{acad.score}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {cv.professionalQualifications && cv.professionalQualifications.length > 0 && (
          <section>
            <h2 className="text-xs font-black uppercase tracking-wider text-teal-800 border-b border-zinc-200 pb-0.5 mb-1.5">
              2. Professional Stages & Papers Cleared
            </h2>
            <div className="space-y-1.5">
              {cv.professionalQualifications.map((pq, idx) => (
                <div key={idx} className="p-2 bg-teal-50/50 rounded border-l-3 border-teal-600 flex justify-between items-baseline text-[10.5px]">
                  <div>
                    <span className="font-bold text-zinc-950">{pq.title}</span>
                    <span className="text-zinc-600 ml-2">({pq.details})</span>
                  </div>
                  {pq.dateInfo && <span className="text-[10px] font-semibold text-teal-800">{pq.dateInfo}</span>}
                </div>
              ))}
            </div>
          </section>
        ...
        {cv.experience && cv.experience.length > 0 && (
          <section>
            <h2 className="text-xs font-black uppercase tracking-wider text-teal-800 border-b border-zinc-200 pb-0.5 mb-1">
              4. Prior Experience & Academic Projects
            </h2>
