/**
 * 05 — MINIMAL ATS
 * Pure single-column layout with zero graphics, sidebars, or icons.
 * Strict standard text hierarchy for 100% ATS parser compatibility.
 * 100% ATS Friendly.
 */
export default function Template05_EmeraldFinance({ cv }) {
  return (
    <div className="w-full bg-white text-black font-sans text-[11px] leading-normal select-text min-h-[1050px] p-8 sm:p-10 text-left">
      ...
      <div className="text-center pb-3 border-b border-black mb-4">
        ...
        <div className="text-xs font-semibold text-gray-800 mt-0.5">
          {[cv.ftsBatch, cv.crn, cv.targetRole || 'Audit & Accounting Professional'].filter(Boolean).join(' | ')}
        </div>
        <div className="text-[10px] text-gray-700 flex justify-center flex-wrap gap-x-4 gap-y-0.5 pt-1">
      ...
      <div className="space-y-3.5">
        ...
        {cv.professionalQualifications && cv.professionalQualifications.length > 0 && (
          <section>
            ...
            <ul className="list-disc pl-5 space-y-0.5 text-[10.5px] text-gray-900">
              {cv.professionalQualifications.map((pq, idx) => (
                <li key={idx}>
                  <strong>{pq.title}</strong> — {pq.details} {pq.dateInfo && <span>({pq.dateInfo})</span>}
                </li>
              ))}
            </ul>
          </section>
        ...
        {cv.academics && cv.academics.length > 0 && (
          <section>
            ...
            <div className="space-y-1 text-[10.5px] text-gray-900">
        ...
        {cv.skills && cv.skills.length > 0 && (
          <section>
            ...
            <p className="text-[10.5px] text-gray-900">{cv.skills.join(' • ')}</p>
          </section>
        )}
