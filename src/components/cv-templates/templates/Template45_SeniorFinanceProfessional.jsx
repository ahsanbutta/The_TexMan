/**
 * 45 — ATS EXECUTIVE
 * ATS-compliant Executive Resume: High machine readability, executive summary section, leadership bullets.
 * 100% ATS Friendly.
 */
export default function Template45_SeniorFinanceProfessional({ cv }) {
  return (
    <div className="w-full bg-white text-black font-sans text-[11px] leading-normal select-text min-h-[1050px] p-8 sm:p-10 text-left">
      <div className="text-center pb-3 border-b-2 border-black mb-4">
        <h1 className="text-2xl font-bold uppercase tracking-wide text-black">{cv.fullName}</h1>
        <div className="text-xs font-semibold text-gray-800 mt-0.5">
          {[cv.ftsBatch, cv.crn, cv.targetRole || 'Chief Financial Officer / Financial Director'].filter(Boolean).join(' | ')}
        </div>
