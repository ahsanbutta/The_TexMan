/**
 * 41 — NO PHOTO PROFESSIONAL
 * Intentionally photo-free clean corporate layout with expansive text width and conservative typography.
 * 100% ATS Friendly.
 */
export default function Template41_ManagementTrainee({ cv }) {
  return (
    <div className="w-full bg-white text-zinc-900 font-serif text-[11px] leading-snug select-text min-h-[1050px] p-8 sm:p-10 text-left">
      <div className="border-b-2 border-zinc-900 pb-3 mb-4 flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold uppercase tracking-tight text-black font-serif">{cv.fullName}</h1>
          <div className="text-xs font-sans font-semibold text-zinc-700 tracking-wider uppercase mt-0.5">
            {[cv.ftsBatch, cv.crn, cv.targetRole || 'Audit & Corporate Advisory Professional'].filter(Boolean).join('  |  ')}
          </div>
        </div>
      ...
      <div className="space-y-3.5 font-sans">
        {cv.personalStatement && (
          <section>
            <h2 className="text-xs font-serif font-bold uppercase tracking-wider text-black border-b border-zinc-300 pb-0.5 mb-1">
              Executive Profile
            </h2>
        ...
        {cv.professionalQualifications && cv.professionalQualifications.length > 0 && (
          <section>
            <h2 className="text-xs font-serif font-bold uppercase tracking-wider text-black border-b border-zinc-300 pb-0.5 mb-1.5">
