/**
 * 47 — ATS CA/ACCA
 * Specialized machine-readable format for CA & ACCA articleship, audit, tax, and accounting trainees.
 * 100% ATS Friendly.
 */
export default function Template47_UltraMinimalATS({ cv }) {
  return (
    <div className="w-full bg-white text-black font-sans text-[11px] leading-normal select-text min-h-[1050px] p-8 sm:p-10 text-left">
      <div className="pb-3 border-b-2 border-black mb-4">
        ...
        <div className="text-xs font-semibold text-gray-800 mt-0.5">
          {[cv.ftsBatch, cv.crn, cv.targetRole || 'CA / ACCA Articleship Trainee'].filter(Boolean).join(' | ')}
        </div>
