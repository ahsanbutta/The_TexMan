...
/**
 * 11 — TAX PROFESSIONAL
 * Tax & Corporate Law specialization with tax compliance badges and litigation advisory structure.
 */
export default function Template11_ModernTwoColumn({ cv }) {
  return (
    <div className="w-full bg-white text-zinc-900 font-sans text-[11px] leading-tight select-text min-h-[1050px] p-7 text-left">
      <div className="border-b-3 border-[#991b1b] pb-4 mb-4 flex justify-between items-center">
        <div className="space-y-1">
          <span className="px-2 py-0.5 bg-red-100 text-[#991b1b] rounded font-bold text-[9px] uppercase tracking-wider">
            Direct & Indirect Taxation Specialist
          </span>
          <h1 className="text-2xl font-black text-black">{cv.fullName}</h1>
          <div className="text-xs text-[#991b1b] font-semibold">
            {[cv.ftsBatch, cv.crn, cv.targetRole || 'Taxation & Legal Consultant'].filter(Boolean).join(' • ')}
          </div>
        </div>
      ...
      <div className="space-y-4">
        {cv.personalStatement && (
          <section>
            <h2 className="text-xs font-black uppercase tracking-wider text-[#991b1b] border-b border-zinc-200 pb-0.5 mb-1">
              Taxation Practice Summary
            </h2>
        ...
        {cv.professionalQualifications && cv.professionalQualifications.length > 0 && (
          <section>
            <h2 className="text-xs font-black uppercase tracking-wider text-[#991b1b] border-b border-zinc-200 pb-0.5 mb-1.5">
              Professional Tax & Accountancy Credentials
            </h2>
            <div className="space-y-1.5">
              {cv.professionalQualifications.map((pq, idx) => (
                <div key={idx} className="p-2 bg-red-50/50 rounded border-l-3 border-[#991b1b] flex justify-between items-baseline text-[10.5px]">
                  <div>
                    <span className="font-bold text-zinc-950">{pq.title}</span>
                    <span className="text-zinc-600 ml-2">({pq.details})</span>
                  </div>
                  {pq.dateInfo && <span className="text-[10px] font-semibold text-[#991b1b]">{pq.dateInfo}</span>}
                </div>
              ))}
            </div>
          </section>
        ...
        {cv.experience && cv.experience.length > 0 && (
          <section>
            <h2 className="text-xs font-black uppercase tracking-wider text-[#991b1b] border-b border-zinc-200 pb-0.5 mb-1">
              Taxation & Corporate Compliance Experience
            </h2>
              ...
              {cv.academics.map((acad, idx) => (
                <div key={idx} className="flex justify-between items-center text-[10.5px] border-b border-zinc-100 pb-1">
                  ...
                  <div className="text-right">
                    <span className="font-bold text-[#991b1b]">{acad.year}</span> | <span className="font-semibold">{acad.score}</span>
                  </div>
                </div>
              ))}
            </div>
